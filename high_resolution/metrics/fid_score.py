...
class FID_Score:
    def __init__(self, dataset_1, dataset_2, device, crop_size=None, generator=None, batch_size=128, dims=2048,
                 num_workers=8, gpu_devices=[]):
    ...
    def compute_fid(self, rtpt=None):
        ...
        # if np.isnan(m1).any() or np.isinf(m1).any() or np.isnan(s1).any() or np.isinf(s1).any():
        #     print("NaN or Inf found in statistics of dataset 1")
        # if np.isnan(m2).any() or np.isinf(m2).any() or np.isnan(s2).any() or np.isinf(s2).any():
        #     print("NaN or Inf found in statistics of dataset 2")
        #
        # # Regularization to ensure covariance matrices are positive definite
        # epsilon = 1e-6
    ...
    def compute_statistics(self, dataset, rtpt=None):
        ...
        def check_images_stats(images):
            print("Max value in batch:", images.max())
            print("Min value in batch:", images.min())
            print("Mean value in batch:", images.mean())
            print("Std deviation in batch:", images.std())
